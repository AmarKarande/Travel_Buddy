    ...
    stages {
            ...
            steps {
                withSonarQubeEnv('SonarQube') {
                    sh '''
                        ...
                        -Dsonar.exclusions="src/assets/images/**,node_modules/**,build/**"
                    '''
                }
            }
        }
        ...
        stage('Docker Build') {
            steps {
                withCredentials([
                    string(
                        credentialsId: 'react-app-env',
                        variable: 'APP_ENV'
                    ),
                    string(
                        credentialsId: 'google-maps-key',
                        variable: 'GOOGLE_MAPS_KEY'
                    ),
                    string(
                        credentialsId: 'rapidapi-key',
                        variable: 'RAPIDAPI_KEY'
                    ),
                    string(
                        credentialsId: 'openweather-key',
                        variable: 'OPENWEATHER_KEY'
                    ),
                    string(
                        credentialsId: 'gemini-key',
                        variable: 'GEMINI_KEY'
                    )
                ]) {
                    sh '''
